#!/usr/local/bin/perl
# Source https://pastebin.com/TU5uV6rX
...
## LOG
$log = '/var/log/ipmi_fan_control/IPMIFanControl.log'; #Ubuntu mod - I personally added my home directory to make it easy to spot check temps, so /home/<username>/IPMIFanControl.log instead
...
$high_cpu_temp = 65;       # will go HIGH when we hit
$med_cpu_temp = 55;        # will go MEDIUM when we hit, or drop below again
$low_cpu_temp = 45;        # will go LOW when we fall below 35 again
...
$hd_ave_target = 41;    # PID control loop will target this average temperature
...
$hd_cpu_override_duty_cycle = 242;  # when the HD duty cycle equals or exceeds this value, the CPU fans may be overridden to help cool HDs
...
$hd_fan_duty_med_high  = 204;    # 80%
$hd_fan_duty_med_low   = 127;    # 50%
$hd_fan_duty_low       = 102;    # 30%, some 120mm fans stall below 30.
$hd_fan_duty_start     = 165;    # 65%, HD fan duty cycle when script starts.
...
# GLOBALS
@hd_list = ("sda", "sdb", "sdc", "sdd", "sde", "sdf");

# massage fan speeds
$cpu_max_fan_speed *= 0.99;
$hd_max_fan_speed *= 0.99;
...
# return minimum, maximum, average HD temperatures and array of individual temps
{
    ...
    {
        my $disk_dev = "/dev/disk/by-id/$item";
...
sub get_fan_mode
{
    ...
    if ($fan_code == 01) { $hd_fan_mode = "Full"; }
    elsif ($fan_code == 00) { $hd_fan_mode = " Std"; }
    elsif ($fan_code == 02) { $hd_fan_mode = " Opt"; }
    elsif ($fan_code == 04) { $hd_fan_mode = " Hvy"; }
...
{
    my $cpu_temp = `$ipmitool sensor get \"CPU1 Temp" | awk '/Sensor Reading/{print \$4}'`;
    chomp $cpu_temp;
